...
class table_mt_wechat_user extends discuz_table {
	public function __construct() {
		$this->_table = 'mt_wechat_user';
		$this->_pk    = 'id';

		parent::__construct();
	}

	public function set_user($weObj,$openid,$event){
		$wechatuser = $this->fetch_by_openid($openid);
		if($event=='subscribe'){
			if($wechatuser){
				$this->update($wechatuser['id'],array('subscribe'=>1,'subscribe_time'=>time()));
			}else{
				$user = $weObj->getUserInfo($openid);
				if($weObj->errCode){
					$this->insert(array('openid' => $openid,'subscribe' => 1,'subscribe_time' => time()));
				}else{
			...
			if($wechatuser){
				$this->update($wechatuser['id'],array('subscribe'=>0,'subscribe_time'=>''));
