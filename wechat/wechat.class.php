...
class Wechat
{
	 ...
	 */
	public function valid($return=false,$accountid)
    {
        $encryptStr="";
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($this->encrypt_type == 'aes') { //aes加密
            	...
            	if (!isset($array[0]) || ($array[0] != 0)) {
            	    if (!$return) {
            	    	C::t('#mt_vote#mt_wechat_account')->update($accountid,array('isconnect' => 0));
            	        die('decrypt error!');
            	    } else {
            	    	C::t('#mt_vote#mt_wechat_account')->update($accountid,array('isconnect' => 0));
            	        return false;
        	...
        	if ($return) {
        		if ($this->checkSignature()){
        			C::t('#mt_vote#mt_wechat_account')->update($accountid,array('isconnect' => 1));
        			return $echoStr;
        		}else{
        			C::t('#mt_vote#mt_wechat_account')->update($accountid,array('isconnect' => 0));
        			return false;
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function getCache($cachename){
		$data = C::t('common_syscache')->fetch($cachename);
	...
	protected function removeCache($cachename){
		C::t('common_syscache')->delete($cachename);
	}
